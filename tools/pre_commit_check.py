...
RULES = [
    ...
    },
    {
        ...
        # ★9/7 [F-189] `0.9x`만 잡아 picks 트랙 C의 `sl = int(x * 0.85)`(−15%)가 두 달 사각이었다 → `0.[89]x`로 확장
        "pattern": r'(?:stop_loss|[a-zA-Z_]*sl)\s*=\s*.*int\(.*\*\s*0\.[89][0-9]',
        "msg": (
            "★ SL 하드코딩 = SAJANG 단일진실 우회 (7/1 전체검수 H1/H2) ★\n"
            "         → sl = int(price * 0.8x/0.9x) 리터럴 = SAJANG 미경유 손절선 (라이브 주문경로면 사고 위험)\n"
