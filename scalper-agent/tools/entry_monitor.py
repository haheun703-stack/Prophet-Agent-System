...
# ─── 장중 실시간 TV 스캔 (v2 — trading_value_scanner 연동) ──

TV_SCAN_INTERVAL = 300  # 5분 (유니버스 전체)
...
def _format_tv_alert(sig) -> str:
    """TV 알림 메시지 포맷 (IntradayTVSignal 호환)"""
    ...
    # IntradayTVSignal
    if hasattr(sig, "estimated_tv_ratio"):
        accel = f" | 가속 {sig.acceleration:.1f}x" if sig.acceleration > 1.1 else ""
        return (
            f"[장중감지] {name} — {tag}\n"
            f"  TV {sig.estimated_tv_ratio:.1f}x | 가격 {sig.change_pct:+.1f}% | score {sig.score:.0f}\n"
            f"  예상{sig.estimated_daily_tv:,.0f}억 (실제{sig.intraday_tv:,.0f}억, 평균{sig.avg_20d_tv:,.0f}억){accel}\n"
            f"  신뢰도: {sig.confidence}"
        )
    # 레거시 TVSignal 호환
    ratio = getattr(sig, "tv_ratio", 0)
    return (
        f"[TV Alert] {name} {tag}!\n"
        f"TV {ratio:.1f}x | 가격 {sig.change_pct:+.1f}% | score {sig.score:.0f}\n"
...
def run_monitor(targets: list[dict], interval: int = 30):
    ...
    # TV 실시간 스캔 상태 (v2 — 가중 시간보정 + 가속도)
    from data.trading_value_scanner import (
        precompute_tv_averages, scan_intraday_universe,
        IntradayHistory, save_intraday_results,
    )
    tv_universe = _load_universe()
    tv_avgs = {}
    tv_prev_codes = set()
    tv_history = IntradayHistory()
    last_tv_scan = 0
    tv_enabled = False
    if len(tv_universe) > 0:
        print("[TV] 20일 평균 거래대금 사전 계산 중...")
        tv_avgs = precompute_tv_averages(tv_universe, min_avg_tv=5.0)
        tv_enabled = len(tv_avgs) > 0
        print(f"[TV v2] 실시간 스캔 대상: {len(tv_avgs)}종목 (가중 시간보정 + 가속도)")
    ...
    while True:
        ...
        # ── 5분 주기 TV 실시간 스캔 (v2: 가중 시간보정 + 가속도) ──
        if tv_enabled and time.time() - last_tv_scan >= TV_SCAN_INTERVAL:
            try:
                new_tv = scan_intraday_universe(
                    trader=trader,
                    tv_avgs=tv_avgs,
                    history=tv_history,
                    prev_alerted=tv_prev_codes,
                    max_scan=500,
                    min_score=60.0,
                )
                if new_tv:
                    save_intraday_results(new_tv)
                    print(f"  [TV v2] 신규 {len(new_tv)}종목 감지!")
                    for sig in new_tv[:5]:
                        msg = _format_tv_alert(sig)
                        print(f"  [TV] {sig.name} {sig.pattern} TV={sig.estimated_tv_ratio:.1f}x "
                              f"score={sig.score:.0f} [{sig.confidence}]")
                        send_telegram(msg)
                        tv_prev_codes.add(sig.code)
                else:
                    if check_count <= 2:
                        print(f"  [TV v2] 실시간 스캔 완료 — 신규 시그널 없음")
            except Exception as e:
                print(f"  [TV v2] 실시간 스캔 오류: {e}")
