# -*- coding: utf-8 -*-
"""[폐기 — 2026-07-24 S-2 D-DAY 판정·사장님 승인] 실행하지 말 것. 재현·감사용 보존.

폐기 사유: 클러스터를 INFRA_KW 종목명 키워드로 하드코딩해 건설 테마가 식은 7/7 이후
13거래일 신호 0 → 표본 9건(승률 11.1%·합 -18.0%p)이 단일 테마·단일 국면에 갇혔다.
게다가 우선주를 별개 회사로 세어(금호건설/금호건설우) 4개 신호 중 3개가 가짜였다.
테마 무관 동적 감지로 재설계해 신호일을 4→46일로 늘려 재검증한 결과(7/24):
  12개월 113건 승률 26.5% -105.13%p / 대조군(클러스터 밖 동일필터) -0.928 vs 전략 -0.930
  → 6·12·24개월 전 구간 미달 + 리프트 0 = "클러스터 소속" 정보 자체에 엣지 없음.
후속 도구: tools/cluster_v2_backtest.py (판정 근거·대조군 방법론 재사용 가능).
nightly ⑯·⑱ 브리핑·⑳ 신선도 배선은 같은 커밋에서 제거됨.

--- 이하 원본 (7/1 사장님 지시) ---
명분 클러스터 매일수확 — forward 페이퍼 트랙.
