...
# 5/9 학습: ewy(NEW) 80%적중 +6.79% → 섹터 보너스 상향
EWY_CORE_SECTORS = {
    "전기전자": 10,   # 삼성전자, SK하이닉스, 삼성SDI, LG에너지솔루션 (EWY 50%+)
    "운송장비": 8,    # 현대차, 기아, 한화에어로 (EWY 10%+)
    "화학": 6,        # LG화학, 에코프로 (EWY 5%+)
    "금속": 6,        # 포스코, LIG넥스원 (방산 포함)
    "기계장비": 4,    # 두산에너빌리티, 한화오션
    "금융": 4,        # KB금융, 신한지주 (EWY 5%+)
    "통신": 4,        # SK텔레콤, KT
    "건설": 4,        # GS건설, 현대건설 (휴전/재건 수혜)
}
