"""
Body Hunter 마스터 오케스트레이터
ETF방향 -> 상대강도 스캔 -> 몸통 포착 -> 자동매매 -> 일일리포트

전체 흐름:
  09:05  ETF 방향 판단 + 상위 5종목 추출
  09:05~ 각 종목 몸통 감시 (동시 다중)
  진입   몸통 탑승
  청산   소진감지 / 트레일링 / SL
  장마감 일일 리포트 + 내일 추천종목
"""
...
from strategies.body_hunter import BodyHunter, BodyState, ExitReason
...
class BodyHunterMaster:
    ...
    def __init__(
        ...
        self.hunters:       Dict[str, BodyHunter]  = {}
    ...
    def scan_candidates(
        ...
        for c in candidates:
            if c.ticker not in self.hunters:
                self.hunters[c.ticker] = BodyHunter(
    ...
    def daily_report(self) -> str:
        ...
        good_exits = sum(1 for r in results if r.exit_reason in ["소진감지", "트레일링"])
