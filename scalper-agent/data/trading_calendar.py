...
KR_HOLIDAYS_2026 = {
    ...
    # ★★9/19 사장님 달력 확인 — **9/28(월)은 대체공휴일이 아니다. 정상 거래일.**
    #   단타봇이 9/7·9/19 **두 번** "9/28 대체공휴일 누락(HIGH)"으로 올렸고 **둘 다 틀렸다**.
    #   근거로 든 것은 *"같은 파일이 이 규칙을 5번 적용했는데 추석만 빠졌다"* 는 패턴 추론이었는데,
    #   **규칙이 공휴일 종류별로 다르다**:
    #     · 설날·추석 연휴 → **일요일**과 겹칠 때만 대체
    #         (설날 1/16금·1/17토·**1/18일** → 일요일 겹침 → 1/19 대체 O)
    #         (추석 9/24목·9/25금·**9/26토** → 일요일 겹침 **없음** → 대체 X)
    #     · 그 밖의 공휴일(삼일절·부처님오신날·광복절·개천절 등) → 토·일 **모두** 대체
    #         (8/15토 → 8/17 O · 10/3토 → 10/5 O)
    #   즉 위 5건은 전부 맞았고, 단타봇이 서로 다른 규칙을 하나로 뭉뚱그렸다.
    #   ★넣었다면 **정상 거래일에 `is_trading_day` 가드가 걸린 잡이 전부 skip** 되어
    #     수집·점검·알림이 통째로 죽는다 — 7/17 제헌절(휴장일을 거래일로 오판)의 **정반대 방향** 사고.
    #   ★아래 [F-114] 주석이 *"음력 기반 설/추석·대체공휴일을 추측해 넣지 마라"* 고
    #     **콕 집어 경고**하고 있었고, 단타봇은 그 주석을 읽은 세션에서 그대로 위반했다.
    #   ⇒ **공휴일 추가는 달력·공시 원본 확인 후에만.** 패턴 추론은 근거가 아니다.
    # 10월
