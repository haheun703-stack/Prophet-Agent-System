...
## ★ 6/1 실측 결과 (노트북, stock_data_daily 4111종목 → 돌파 53,627 이벤트) ★

> 클라우드 세션은 데이터 없어 로직만 검증 → **노트북 세션이 실데이터로 실측**.
> 실데이터 헤더 호환 보정 추가(단타봇): 날짜 컬럼이 파일마다 `날짜`/`Date`/빈헤더(`,open..`)로 상이 →
> `_load` 다중키 폴백 + `utf-8-sig`(BOM) 처리. 이 보정 전엔 OOS가 최신상장주 987건(편향)만 잡았음.

| 항목 | 결과 |
|---|---|
| **[C] vs STOP_ONCE break-even** | **2.41%/leg = 현 0.48% 대비 5.0배 헤드룸** (vs HOLD 9.53% = 19.9배) |
| **[D] 끼 등급별 헤드룸 (단조↑)** | SLUGGISH 3.7x → MODERATE 6.4x → HUNTABLE 9.4x → **EXPLOSIVE 12.2x** |
| **[E] OOS 전/후반 (median 2022-05-17, 반반분할)** | 전반 26,796건 **4.7배** · 후반 26,831건 **5.4배** (양기간 ≥2배 = 레짐 비의존) |

**→ 판정: vs ONCE 5.0배 ≥2배 ✅ AND OOS 양기간 모두 ≥1배(실제 ≥2배) ✅ = 비용·레짐에 강건 → paper 진행 신호 (비용 차원 통과).**

정직 한계(불변): 일봉 해상도(분봉 슬리피지 미반영 — 숙제 #2), 생존편향(절대 BE 과대 → 배수만 신뢰), 종가체결 가정.
즉 **비용 차원은 통과**지만, paper 최종 go는 #2 분봉정밀·#3 생존편향보정 후 종합 판정.
