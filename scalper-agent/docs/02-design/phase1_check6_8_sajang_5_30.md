...
## ★ 대전제 — 매도 철학 (5/30 사장님 정정) ★

> **"고정 +5% TP 제거 = 매도 제거"가 아니다. "고정 TP 제거 + 동적 단타 청산 강화"가 목표.**
> 이 봇은 당일~D+3 안에 **수익을 실현하는 단타봇**. CHECK-6 치환 시 트레일링·D+0~D+3 청산은 **죽이지 말고 강화**.

매도 8원칙: ①맥락없는 고정익절만 금지 ②트레일링 익절 강화 ③진입 -3% 손절 ④+3%↑ 고점-3% 트레일링 ⑤마감 전 당일/익일 판단 ⑥강한종목만 D+1~D+3 ⑦D+3 초과 금지/승인 ⑧상한가·거래대금·수급·갭·호가붕괴·룰B/C/D 반영.
→ SAJANG에 없는 헬퍼(D+3 한도 등)는 신규 추가 = 사장님 승인. 기존 청산 로직은 삭제 아니라 **SAJANG 승격**.

**역할**: 리딩=단타봇(방향·우선순위·검증기준) / 구현=Codex(삭제·교체·재구성 허용) / 검증=단타봇(audit·paper·intent 대사, 코드 직접수정 X).
