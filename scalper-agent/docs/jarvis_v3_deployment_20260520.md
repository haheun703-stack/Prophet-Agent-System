...
## 6. 사장님 결정 확정 (5/19 12:00)

### ✅ Q1: 5/20 가동 방식 → **B 진보 (wire-up 자동 가동)**
- `asset_pool_scan_and_buy(top_k=5, qty_per_stock=1)` 09:05 1회 호출
- advisory BEARISH/PANIC 시 자동 차단 (안전망)

### ✅ Q2: 매수 수량 → **1주 모드 유지**
- 5/19 검증 결과 반영: 1주 모드라야 -16.9만 같은 잠재 손실도 -3만/종목으로 제한
- 5/21 이후 강세장 확인 시 다주 모드 검토

### ✅ Q3: 알고리즘 A 텔레그램 → **즉시 활성화**
- 09:30 / 11:00 / 13:00 KST 3회 자동 호출
- 같은 섹터 4개+ 동시 +10%+ 시 즉시 텔레그램 알림

### ✅ 결함 2 wire-up → **5/20 새벽 영구 배포**
- 이미 trading_coo.py 스케줄러에 등록 (5/19 12:00 완료)

---

## 7. 5/20 가동 시나리오 시간표 (B+1주 확정)

```
06:00~06:30  사장님: 데이터 확인 (brain_state / 막내 / 큰형 advisory)
06:30~07:30  VPS git pull → systemctl restart bodyhunter-bot
07:30~08:30  사장님: 검증 3건 (KIS strength / 알고리즘 A / asset_pool)
08:54 전     사장님: 텔레그램 시작 버튼

09:00        morning_rec 13종목 매수 (1주씩)
09:05        ★ asset_pool TOP 5 × 1주 자동 매수 (advisory 게이트 통과 시) ★
09:05~14:00  intraday_scanner 5분마다 (멀티시그널 1주씩)
09:30        섹터 동조 카운터 1회차 (4그룹+ 시 알림)
11:00        섹터 동조 카운터 2회차
13:00        섹터 동조 카운터 3회차
09:05+ 매수 후 +5~8% → 즉시 익절 (자비스 v2 회전)
14:00        intraday 신규 진입 차단 (안전망)
15:25        verification 보유 전체 청산
15:30        장 마감
15:35        검증모드 정산
16:30        G7 EVENING_BRAIN
```
...
| 기준 | 목표 | 평가 |
|------|------|------|
| advisory 게이트 작동 | BEARISH 시 0매수 | ⬜ |
| asset_pool 1주씩 매수 | top_k=5 종목 | ⬜ |
| 익절 회전 횟수 | 5회+ (사장님 야간 패러다임) | ⬜ |
| 평가손익 | +3만 ~ +10만 (1주 모드 정상) | ⬜ |
| 손실 한도 | -10만 미만 | ⬜ |
| 섹터 동조 알림 | 정확도 90%+ | ⬜ |
